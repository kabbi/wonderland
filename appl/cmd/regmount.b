...
reader(fd: ref sys->FD, cstring: chan of IPaddr, cpid: chan of int)
{
	...
	while (1) {
		...
		sys->print("Answer from: %s, mounting\n", string buf[Udphdrsize:i]);
		hdr := Udphdr.unpack(buf[:Udphdrsize], Udphdrsize);
		
		# filter bad symbols
		for (k := Udphdrsize; k < i; k++)
			if (buf[k] == byte '\'')
				buf[k] = byte ' ';
		# run the mount command
		mountcmd := "mount 'tcp!" + hdr.raddr.text() + "!registry' /mnt/registries/" + string buf[Udphdrsize:i-4];
		sh->system(context, mountcmd);
